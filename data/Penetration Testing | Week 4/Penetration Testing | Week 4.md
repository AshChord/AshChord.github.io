...
127.0.0.1(로컬)의 8080 포트에 리스너가 설정되어 있으므로, 클라이언트의 트래픽이 127.0.0.1:8080으로 도착하면 프록시 툴이 이를 관리할 수 있다.

프록시 리스너를 추가할 경우 다음과 같은 바인딩 옵션에 유의하여야 한다.

<img src="/data/Penetration%20Testing%20%7C%20Week%204/4.png" alt="프록시 리스너 바인딩 옵션" width="600">
...
프록시 리스너를 설정하였다면, 웹 브라우저에서 프록시 설정을 통해 프록시 서버를 지정해주어야 한다.  
Chrome에서 [설정] - [시스템] - [컴퓨터 프록시 설정 열기]를 선택하여 다음과 같이 설정한다.

<img src="/data/Penetration%20Testing%20%7C%20Week%204/5.png" alt="프록시 설정" width="500">

이후 브라우저에서 웹 사이트에 접속하면, 다음과 같이 Burp Suite에서 트래픽을 확인할 수 있다.

![프록시 설정 확인](/data/Penetration%20Testing%20|%20Week%204/6.png)

민약 Burp Suite를 켜지 않은 채로 웹 페이지를 요청하면, `ERR_PROXY_CONNECTION_FAILED` 에러가 발생한다.

![프록시 연결 에러](/data/Penetration%20Testing%20|%20Week%204/7.png)

<br>

#### Burp Suite Practice Challenges

Burp Suite 사용에 익숙해지기 위해 SegFault 실습 사이트의 Burp Suite Practice Challenges를 해결해 보자.

<br>

##### Burp Suite Prac 1

<img src="/data/Penetration%20Testing%20%7C%20Week%204/8.png" alt="Prac 1" width="400">

링크의 주소로 접속하면 다음과 같은 웹 페이지로 이동한다.

<img src="/data/Penetration%20Testing%20%7C%20Week%204/9.png" alt="Prac 1" width="600">

표면상으로는 아무런 데이터가 없는 듯 보였지만 확인 차 소스 코드를 살펴 본 결과 다음과 같았다.

<img src="/data/Penetration%20Testing%20%7C%20Week%204/10.png" alt="Prac 1" width="600">

HTTP 요청의 헤더를 조작해야 하므로 Burp Suite의 Intercept 기능을 활용해 데이터 패킷을 가로채고 내용을 확인하였다.

![Prac 1](/data/Penetration%20Testing%20|%20Week%204/11.png)

User-Agent 필드의 값을 segfaultDevice로 수정 후 요청을 서버로 전달(Forward)하였다.

![Prac 1](/data/Penetration%20Testing%20|%20Week%204/12.png)

이후 다음과 같은 메시지가 표시되며, 소스 코드를 확인하여 플래그를 획득하였다.

<img src="/data/Penetration%20Testing%20%7C%20Week%204/13.png" alt="Prac 1" width="600">
<img src="/data/Penetration%20Testing%20%7C%20Week%204/14.png" alt="Prac 1" width="600">
...
<img src="/data/Penetration%20Testing%20%7C%20Week%204/15.png" alt="Prac 2" width="400">

링크의 주소로 접속하면 다음과 같은 웹 페이지로 이동한다.

<img src="/data/Penetration%20Testing%20%7C%20Week%204/16.png" alt="Prac 2" width="600">
<img src="/data/Penetration%20Testing%20%7C%20Week%204/17.png" alt="Prac 2" width="600">

소스 코드에는 일부 내용이 작성되어 있었으나, 인코딩 오류인지 한글이 정상적으로 표시되지 않았다. LOOK INSIDE 메시지를 바탕으로 Burp Suite 내부에서 서버의 응답 내용을 확인해 보았다.

<img src="/data/Penetration%20Testing%20%7C%20Week%204/18.png" alt="Prac 2" width="600">

그 결과, `a.html`과 `b.html` 파일의 데이터를 확인해보라는 메시지가 소스 코드에 숨겨져 있음을 확인할 수 있었다. 현재 웹 페이지의 경로가 `ctf.segfaulthub.com:1019/2_burp/` 형태였으므로, 해당 경로 뒤에 각각 `a.html`과 `b.html`을 추가하여 접근하였다.

<img src="/data/Penetration%20Testing%20%7C%20Week%204/19.png" alt="Prac 2" width="600">
<img src="/data/Penetration%20Testing%20%7C%20Week%204/20.png" alt="Prac 2" width="600">
...
<img src="/data/Penetration%20Testing%20%7C%20Week%204/22.png" alt="Prac 3" width="400">

링크의 주소로 접속하면 다음과 같은 웹 페이지로 이동한다.

<img src="/data/Penetration%20Testing%20%7C%20Week%204/23.png" alt="Prac 3" width="600">

소스 코드에도 별다른 내용은 없었으므로, 바로 Burp Suite에서 패킷을 분석해 보았다.

![Prac 3](/data/Penetration%20Testing%20|%20Week%204/24.png)
...
![Prac 3](/data/Penetration%20Testing%20|%20Week%204/25.png)

새로고침 이후 요청에 `Cookie` 헤더가 생성되었으며 마찬가지로 `answer=1`값이 설정되어 있음을 확인할 수 있었다. 1~20이라는 힌트로 미루어보아 `answer`에 1부터 20까지의 값을 넣어서 전송하면 플래그를 얻을 수 있을 것으로 예측되었고, 해당 패킷을 Repeater로 전달하였다.

<img src="/data/Penetration%20Testing%20%7C%20Week%204/26.png" alt="Prac 3" width="300">

Repeater에서 `answer` 값을 변경하면서 반복적으로 요청을 전송하였더니, `answer=13`일 때 플래그가 출력되는 것을 확인할 수 있었다.

![Prac 3](/data/Penetration%20Testing%20|%20Week%204/27.png)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%204/28.png" alt="Prac 4" width="400">

링크의 주소로 접속하면 다음과 같은 웹 페이지로 이동한다.

<img src="/data/Penetration%20Testing%20%7C%20Week%204/29.png" alt="Prac 4" width="600">

마찬가지로 소스 코드에는 별다른 내용이 없어 바로 패킷 분석을 진행하였다.

![Prac 4](/data/Penetration%20Testing%20|%20Week%204/30.png)

요청을 자세히 살펴 보니 `Cookie` 헤더에 `level=dXNlcg%3D%3D`이라는 값이 존재함을 알 수 있었다.  
인코딩된 텍스트라고 생각되어 Decoder 탭에서 디코딩을 진행하였다.

![Prac 4](/data/Penetration%20Testing%20|%20Week%204/31.png)
...
![Prac 4](/data/Penetration%20Testing%20|%20Week%204/32.png)
![Prac 4](/data/Penetration%20Testing%20|%20Week%204/33.png)

`WXpKV2JscHRSakZpU0ZJM1VrZFdhbUl5VW14VmJWWjNXbGRHTUdaUlBUMD0=`라는 텍스트가 출력되었다.  
마찬가지로 디코딩을 진행해 보았다.

![Prac 4](/data/Penetration%20Testing%20|%20Week%204/34.png)
