...
![XSS CTF](/data/Penetration%20Testing%20%7C%20Week%209/1.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%209/2.webp" alt="XSS 1" style="padding: 0 25%; background-color: white">

링크의 주소로 접속하면 SQL Injection Advanced CTF에서와 같은 회원제 게시판 애플리케이션으로 이동한다.

![XSS 1](/data/Penetration%20Testing%20%7C%20Week%209/3.webp)

XSS 공격의 특성을 고려하면, 작성한 글이 화면에 출력되는 게시판에 취약점이 존재할 확률이 높다. 따라서 우선 회원가입을 진행해 `any`/`any` 계정을 생성한 후, 로그인하여 게시판에 글을 작성해 보았다.

![XSS 1](/data/Penetration%20Testing%20%7C%20Week%209/4.webp)

게시판의 동작 방식에 따라 입력한 데이터가 자연스럽게 게시물 목록에 출력되는 것을 알 수 있다.

![XSS 1](/data/Penetration%20Testing%20%7C%20Week%209/5.webp)
![XSS 1](/data/Penetration%20Testing%20%7C%20Week%209/6.webp)
...
![XSS 1](/data/Penetration%20Testing%20%7C%20Week%209/7.webp)
...
![XSS 1](/data/Penetration%20Testing%20%7C%20Week%209/8.webp)
...
![XSS 1](/data/Penetration%20Testing%20%7C%20Week%209/9.webp)

서버 응답에 스크립트가 정상적으로 포함되어 있는 것을 확인하였다.

![XSS 1](/data/Penetration%20Testing%20%7C%20Week%209/10.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%209/11.webp" alt="XSS 2" style="padding: 0 25%; background-color: white">

링크의 주소로 접속한 후, 우선 제목과 내용이 `<'">`인 게시물을 생성해 보았다.

![XSS 2](/data/Penetration%20Testing%20%7C%20Week%209/12.webp)
![XSS 2](/data/Penetration%20Testing%20%7C%20Week%209/13.webp)
...
![XSS 2](/data/Penetration%20Testing%20%7C%20Week%209/14.webp)

그러나 검색어는 화면에 출력되지 않았다. 따라서 예상했던 동작 방식이 실제 구현과 다르다는 사실을 확인하였으나, 검색 결과가 반환되지 않은 점에 의문이 들어 패킷을 확인해 보았다.

![XSS 2](/data/Penetration%20Testing%20%7C%20Week%209/15.webp)
...
<pre><button class="copy-button"></button><code class="language-js" highlighted><data class="code-line" value="1"><span class="hljs-tag">&lt;<span class="hljs-name">script</span>&gt;</span><span class="language-xml">
</data><data class="code-line" value="2" style="--indent: 2ch;">  <span class="hljs-title function_">alert</span>(<span class="hljs-string">'&amp;lt;'</span><span class="hljs-string">"&amp;gt;에 대한 검색 결과가 존재하지 않습니다.');
</data><data class="code-line" value="3"></span><span class="hljs-tag">&lt;/<span class="hljs-name">script</span>&gt;
</data></code></pre>
...
</data><data class="code-line" value="2" style="--indent: 2ch;">  <span class="hljs-title function_">alert</span>(<span class="hljs-string">''</span>);
</data><data class="code-line" value="3" style="--indent: 2ch;">  <span class="hljs-title function_">alert</span>(<span class="hljs-number">1</span>);
</data><data class="code-line" value="4" style="--indent: 2ch;">  (<span class="hljs-string">'에 대한 검색 결과가 존재하지 않습니다.'</span>); <span class="hljs-comment">// 어떠한 동작도 수행하지 않는 무의미한 문자열 표현식</span>
</data><data class="code-line" value="5"><span class="hljs-tag">&lt;/<span class="hljs-name">script</span>&gt;</span>
</data></code></pre>

![XSS 2](/data/Penetration%20Testing%20%7C%20Week%209/16.webp)
...
![XSS 2](/data/Penetration%20Testing%20%7C%20Week%209/17.webp)

요청 전송 시 우클릭을 통해 컨텍스트 메뉴를 열면 요청 방식을 변경할 수 있는 항목이 존재한다.

![XSS 2](/data/Penetration%20Testing%20%7C%20Week%209/18.webp)

요청 방식을 GET 방식으로 변경하여 전송하면 정상적으로 서버 응답이 반환되는 것을 확인할 수 있으며, 해당 요청의 URL 역시 추출할 수 있다.

![XSS 2](/data/Penetration%20Testing%20%7C%20Week%209/19.webp)
![XSS 2](/data/Penetration%20Testing%20%7C%20Week%209/20.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%209/21.webp" alt="XSS 3" style="padding: 0 25%; background-color: white">
...
![XSS 3](/data/Penetration%20Testing%20%7C%20Week%209/22.webp)
...
![XSS 3](/data/Penetration%20Testing%20%7C%20Week%209/23.webp)
...
<pre><button class="copy-button"></button><code class="language-js" highlighted><data class="code-line" value="1"><span class="hljs-tag">&lt;<span class="hljs-name">input</span> <span class="hljs-attr">name</span> = <span class="hljs-string">"id"</span> <span class="hljs-attr">type</span> = <span class="hljs-string">"text"</span> <span class="hljs-attr">placeholder</span>=<span class="hljs-string">""</span>/&gt;</span>
</data><data class="code-line" value="2"><span class="hljs-tag">&lt;<span class="hljs-name">script</span>&gt;</span>
</data><data class="code-line" value="3" style="--indent: 2ch;">  <span class="hljs-title function_">alert</span>(<span class="hljs-number">1</span>);
</data><data class="code-line" value="4"><span class="hljs-tag">&lt;/<span class="hljs-name">script</span>&gt;</span>
</data><data class="code-line" value="5">&lt;"/&gt; <span class="hljs-comment">// 존재하지 않는 태그로 일반 텍스트처럼 처리됨</span>
</data></code></pre>

![XSS 3](/data/Penetration%20Testing%20%7C%20Week%209/24.webp)

서버 응답에 스크립트가 정상적으로 포함되는 것을 확인하였다. 이 방식 또한 Reflected XSS에 해당하므로, 해당 요청의 URL로 접속해 스크립트가 실행되는지 재확인해 보았다.

![XSS 3](/data/Penetration%20Testing%20%7C%20Week%209/25.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%209/26.webp" alt="XSS 4" style="padding: 0 25%; background-color: white">

링크의 주소로 접속하여 제목과 내용이 `<'">`인 게시물을 생성해 보았다.

![XSS 4](/data/Penetration%20Testing%20%7C%20Week%209/27.webp)
![XSS 4](/data/Penetration%20Testing%20%7C%20Week%209/28.webp)
...
![XSS 4](/data/Penetration%20Testing%20%7C%20Week%209/29.webp)
...
![XSS 4](/data/Penetration%20Testing%20%7C%20Week%209/30.webp)

예상한대로 `<script>` 태그가 정상적으로 반환되었다. 다음으로 `<scrscriptipt>alert(1);</scrscriptipt>`을 사용하여 스크립트 실행을 시도하였다. 

![XSS 4](/data/Penetration%20Testing%20%7C%20Week%209/31.webp)

`alert` 문자열 역시 필터링되고 있음을 확인하였다. 마찬가지 방법으로 `<scrscriptipt>alalertert(1);</scrscriptipt>`를 대신 사용하였다.

![XSS 4](/data/Penetration%20Testing%20%7C%20Week%209/32.webp)

서버 응답에 스크립트가 정상적으로 포함되어 있는 것을 확인하였다.

![XSS 4](/data/Penetration%20Testing%20%7C%20Week%209/33.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%209/34.webp" alt="XSS 5" style="padding: 0 25%; background-color: white">
...
![XSS 5](/data/Penetration%20Testing%20%7C%20Week%209/35.webp)
...
![XSS 5](/data/Penetration%20Testing%20%7C%20Week%209/36.webp)
![XSS 5](/data/Penetration%20Testing%20%7C%20Week%209/37.webp)
...
![XSS 5](/data/Penetration%20Testing%20%7C%20Week%209/38.webp)

서버 응답에 스크립트가 정상적으로 포함되어 있는 것을 확인하였다.

![XSS 5](/data/Penetration%20Testing%20%7C%20Week%209/39.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%209/40.webp" alt="XSS 6" style="padding: 0 25%; background-color: white">

링크의 주소로 접속하여 게시판 및 마이페이지를 면밀히 조사해 보았지만 XSS 취약점을 발견할 수 없었다.

마지막으로 XSS 공격의 가능성을 확인한 곳은 다름 아닌 `index.php` 페이지였다.

![XSS 6](/data/Penetration%20Testing%20%7C%20Week%209/41.webp)
...
![XSS 6](/data/Penetration%20Testing%20%7C%20Week%209/42.webp)

회원 가입에 성공하였다. 이후 로그인 페이지에서 `<script>alert(1);</script>`/`any`를 입력하여 로그인을 시도하였다.

![XSS 6](/data/Penetration%20Testing%20%7C%20Week%209/43.webp)
...
![XSS 6](/data/Penetration%20Testing%20%7C%20Week%209/44.webp)

서버 응답에 스크립트가 정상적으로 포함되는 것을 확인하였다. 이 방식 또한 Reflected XSS에 해당하므로, 해당 요청의 URL로 접속해 스크립트가 실행되는지 재확인해 보았다.

![XSS 6](/data/Penetration%20Testing%20%7C%20Week%209/45.webp)
![XSS 6](/data/Penetration%20Testing%20%7C%20Week%209/46.webp)
