...
<img src="/data/Penetration%20Testing%20%7C%20Week%209/2.png" alt="XSS 1" style="border-right: 200px solid #f3f5fa; border-left: 200px solid #f3f5fa;">
...
<img src="/data/Penetration%20Testing%20%7C%20Week%209/11.png" alt="XSS 2" style="border-right: 200px solid #f3f5fa; border-left: 200px solid #f3f5fa;">
...
`option_val` 파라미터 중 `username` 값이 설정되어 있음을 확인하고 검색 과정에서 착오가 있었음을 인지하였다. 검색 기능이 기본적으로 작성자를 기준으로 동작하도록 되어 있었기 때문에, 제목이 `<'">`인 게시물이 검색되지 않았던 것이다.
...
<img src="/data/Penetration%20Testing%20%7C%20Week%209/21.png" alt="XSS 3" style="border-right: 200px solid #f3f5fa; border-left: 200px solid #f3f5fa;">
...
마이페이지 접속 요청을 변조하여 `user`를 `<'">`로 변경하였고, 그 결과 아이디 란의 플레이스홀더에 입력한 텍스트 `<'">`가 그대로 출력되는 것을 확인하였다. 모든 문자가 별도의 변환 처리 없이 전송되는 것을 확인했으므로, 이번에는 `user`를 다음과 같이 변경하였다.

```text
"/><script>alert(1);</script><
```

이때 아이디 란에 해당하는 `input` 태그는 다음과 같이 변경된다.
...
<img src="/data/Penetration%20Testing%20%7C%20Week%209/26.png" alt="XSS 4" style="border-right: 200px solid #f3f5fa; border-left: 200px solid #f3f5fa;">
...
<img src="/data/Penetration%20Testing%20%7C%20Week%209/34.png" alt="XSS 5" style="border-right: 200px solid #f3f5fa; border-left: 200px solid #f3f5fa;">
...
해당 Javascript 코드를 제거하고 다시 제목과 내용이 `<'">`인 게시물을 생성한 결과, 내용 란에 작성한 특수문자가 그대로 출력되는 것을 확인하였다. 다른 위치에서는 서버 측에서도 HTML 인코딩이 수행되었으나, 게시물 내용은 클라이언트 측에서 실행되는 인코딩 스크립트에 의해서만 특수 문자 변환이 이루어지고 있었다. 따라서 게시물 내용에 `<script>alert(1);</script>`를 입력해 보았다.
...
<img src="/data/Penetration%20Testing%20%7C%20Week%209/40.png" alt="XSS 6" style="border-right: 200px solid #f3f5fa; border-left: 200px solid #f3f5fa;">
