...
![Description](/data/Dreamhack%20Wargames%20%7C%20Dream%20Beginners%20I/1.webp)
...
![Write-Up](/data/Dreamhack%20Wargames%20%7C%20Dream%20Beginners%20I/2.webp)
...
![Description](/data/Dreamhack%20Wargames%20%7C%20Dream%20Beginners%20I/3.webp)
...
![Write-Up](/data/Dreamhack%20Wargames%20%7C%20Dream%20Beginners%20I/4.webp)
...
![Write-Up](/data/Dreamhack%20Wargames%20%7C%20Dream%20Beginners%20I/5.webp)
...
![Description](/data/Dreamhack%20Wargames%20%7C%20Dream%20Beginners%20I/6.webp)
...
![Write-Up](/data/Dreamhack%20Wargames%20%7C%20Dream%20Beginners%20I/12.webp)

첨부 파일을 다운로드하고 압축을 해제하면 `app.py` 파일을 확인할 수 있다. 참고로 웹 페이지에서 `cat app.py`를 입력해도 동일한 소스 코드가 출력된다.

코드를 살펴보면 Python의 웹 프레임워크인 Flask를 사용해 개발된 웹 서비스임을 확인할 수 있다.  
...
`index()` 함수는 기본적으로 `index.html`을 반환한다. 그러나 POST 요청(사용자가 입력 양식을 통해 값 제출)을 수신한 경우, `index()`는 사용자 입력값을 받아 `echo $(...)` 형태의 명령어를 셸에서 실행한 뒤 `result`에 그 결과를 지정하여 `index.html`을 반환한다.

> **`subprocess`**: Python에서 외부 명령어를 실행시키는 모듈  
> **`subprocess.check_output()`**: 명령의 출력값을 반환하는 함수  
> **`/bin/sh`**: 리눅스의 기본 셸 프로그램  
> **`-c`**: 셸 명령어를 문자열로 전달하여 실행시키는 옵션
...
![Write-Up](/data/Dreamhack%20Wargames%20%7C%20Dream%20Beginners%20I/13.webp)
