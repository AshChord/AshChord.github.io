...
![SQL Injection Advanced CTF](/data/Penetration%20Testing%20%7C%20Week%208/1.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%208/2.webp" alt="SQL Injection Point 1" style="padding: 0 25%; background-color: white">

링크의 주소로 접속하면 다음과 같은 웹 페이지로 이동한다.

![SQL Injection Point 1](/data/Penetration%20Testing%20%7C%20Week%208/3.webp)
...
![SQL Injection Point 1](/data/Penetration%20Testing%20%7C%20Week%208/4.webp)
...
![SQL Injection Point 1](/data/Penetration%20Testing%20%7C%20Week%208/5.webp)
...
![SQL Injection Point 1](/data/Penetration%20Testing%20%7C%20Week%208/6.webp)
...
![SQL Injection Point 1](/data/Penetration%20Testing%20%7C%20Week%208/7.webp)
...
![SQL Injection Point 1](/data/Penetration%20Testing%20%7C%20Week%208/8.webp)
![SQL Injection Point 1](/data/Penetration%20Testing%20%7C%20Week%208/9.webp)

컬럼 수가 오직 1개임을 파악하였다. 그렇다면 해당 컬럼의 값이 곧 기타 정보 필드에 출력되는 것임을 의미한다. 따라서 쿠키의 user 값을 `any' union select database()`로 수정해 보았다.

![SQL Injection Point 1](/data/Penetration%20Testing%20%7C%20Week%208/10.webp)

성공적으로 현재 데이터베이스명이 출력되었다. 이후 데이터 추출을 위한 UNION SQL Injection 절차를 순서대로 수행하였다.

![SQL Injection Point 1](/data/Penetration%20Testing%20%7C%20Week%208/11.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%208/12.webp" alt="SQL Injection Point 2" style="padding: 0 25%; background-color: white">
...
![SQL Injection Point 2](/data/Penetration%20Testing%20%7C%20Week%208/13.webp)

우선 예시로 `test` 게시물을 생성한 후, 게시물 검색 기능을 사용해 보았다.

![SQL Injection Point 2](/data/Penetration%20Testing%20%7C%20Week%208/14.webp)
...
![SQL Injection Point 2](/data/Penetration%20Testing%20%7C%20Week%208/15.webp)
...
![SQL Injection Point 2](/data/Penetration%20Testing%20%7C%20Week%208/16.webp)

정상적으로 검색 결과가 출력되었다.

![SQL Injection Point 2](/data/Penetration%20Testing%20%7C%20Week%208/17.webp)
...
![SQL Injection Point 2](/data/Penetration%20Testing%20%7C%20Week%208/18.webp)
![SQL Injection Point 2](/data/Penetration%20Testing%20%7C%20Week%208/19.webp)
...
![SQL Injection Point 2](/data/Penetration%20Testing%20%7C%20Week%208/20.webp)

1, 2, 3, 4번째 컬럼이 출력되는 것을 확인하였다. 이후 데이터베이스명, 테이블명, 컬럼명을 순차적으로 추출하고 플래그 획득에 성공하였다.

![SQL Injection Point 2](/data/Penetration%20Testing%20%7C%20Week%208/21.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%208/22.webp" alt="SQL Injection Point 3" style="padding: 0 25%; background-color: white">

링크의 주소로 접속하면 마찬가지로 회원제 게시판 애플리케이션으로 이동한다.

전체적인 구조를 살펴보던 중 앞선 문제들과는 다른 점을 발견하였다.

![SQL Injection Point 3](/data/Penetration%20Testing%20%7C%20Week%208/23.webp)
...
![SQL Injection Point 3](/data/Penetration%20Testing%20%7C%20Week%208/24.webp)

`sort` 파라미터에 삽입한 페이로드는 조건이 참이므로 `1`의 값으로 변환되며, 게시물 검색 결과를 테이블의 첫 번째 컬럼을 기준으로 정렬시킨다. 조건식을 `1 = 2`로 변경하면 어떻게 될까?

![SQL Injection Point 3](/data/Penetration%20Testing%20%7C%20Week%208/25.webp)
...
![SQL Injection Point 3](/data/Penetration%20Testing%20%7C%20Week%208/26.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%208/27.webp" alt="SQL Injection Point 4" style="padding: 0 25%; background-color: white">

링크의 주소로 접속하면 마찬가지로 회원제 게시판 애플리케이션으로 이동한다.

SQL Injection 취약점을 탐색하던 도중 SQL Injection Point 2에서 발견했던 위치에서 동일하게 취약점이 존재한다는 것을 확인하였다.

![SQL Injection Point 4](/data/Penetration%20Testing%20%7C%20Week%208/28.webp)
![SQL Injection Point 4](/data/Penetration%20Testing%20%7C%20Week%208/29.webp)

`ORDER BY` 절을 사용하여 테이블의 컬럼 수가 10개인 것을 확인하였고, 마찬가지로 UNION SQL Injection을 시도해 보았다.

![SQL Injection Point 4](/data/Penetration%20Testing%20%7C%20Week%208/30.webp)
...
![SQL Injection Point 4](/data/Penetration%20Testing%20%7C%20Week%208/31.webp)
...
![SQL Injection Point 4](/data/Penetration%20Testing%20%7C%20Week%208/32.webp)
...
![SQL Injection Point 5](/data/Penetration%20Testing%20%7C%20Week%208/33.webp)
...
![SQL Injection Point 5](/data/Penetration%20Testing%20%7C%20Week%208/34.webp)

컬럼명 `flag`가 추출되었고, `option_val` 파라미터에 최종적으로 `1 = 1 union select 1, flag, 3, 4, 5, 6, 7, 8, 9, 10 from flagHere#`를 입력하여 플래그를 획득하였다.

![SQL Injection Point 5](/data/Penetration%20Testing%20%7C%20Week%208/35.webp)
