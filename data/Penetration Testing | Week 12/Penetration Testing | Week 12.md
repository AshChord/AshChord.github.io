...
이때는 반드시 XSS 취약점을 연계하여 공격을 시도해야 하며, 일반적으로 `<form>` 태그를 이용하는 방식이 자주 활용된다. 위의 인터넷 뱅킹 서비스 예시에서 서버가 POST 방식을 사용한다고 가정하고 다음의 예시 코드를 살펴보자.
...
그러나 사용자가 버튼을 클릭하도록 유도하는 행위는 링크를 전달하는 경우와 마찬가지로 사용자의 경계심을 유발할 수 있다. 따라서 이번에도 사용자의 개입 없이 자동으로 요청을 전송시키는 방식을 고려할 필요가 있다. 다음의 코드를 보자.
...
![CSRF CTF](/data/Penetration%20Testing%20%7C%20Week%2012/1.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%2012/2.webp" alt="GET Admin 1" style="padding: 0 25%; background-color: white;">

링크의 주소로 접속하면 회원제 게시판 애플리케이션으로 이동한다.

회원 가입을 진행하여 `any`/`any` 계정을 생성하고, 로그인 후 계정 정보가 포함된 마이페이지로 이동하였다.

![GET Admin 1](/data/Penetration%20Testing%20%7C%20Week%2012/3.webp)
...
![GET Admin 1](/data/Penetration%20Testing%20%7C%20Week%2012/4.webp)
![GET Admin 1](/data/Penetration%20Testing%20%7C%20Week%2012/5.webp)
...
![GET Admin 1](/data/Penetration%20Testing%20%7C%20Week%2012/6.webp)

그 결과 관리자가 알아차렸다는 메시지가 출력되었다. 이에 대해 고민해 본 결과, 서버 측 응답에서 그 이유를 파악할 수 있었다.

<img src="/data/Penetration%20Testing%20%7C%20Week%2012/7.webp" alt="GET Admin 1" style="padding: 0 12%; background-color: #2b2b2b;">
...
![GET Admin 1](/data/Penetration%20Testing%20%7C%20Week%2012/8.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%2012/12.webp" alt="GET Admin 2" style="padding: 0 25%; background-color: white;">

링크의 주소로 접속하여 `any`/`any` 계정을 생성해 로그인한 후, 마이페이지에서 비밀번호를 변경해 보았다.

![GET Admin 2](/data/Penetration%20Testing%20%7C%20Week%2012/13.webp)
![GET Admin 2](/data/Penetration%20Testing%20%7C%20Week%2012/14.webp)

이번에는 서버가 POST 방식의 요청만을 허용하고 있다는 사실을 알 수 있었다. 따라서 `<form>` 태그를 통한 공격을 활용하기로 결정하고 XSS 취약점을 탐색해 보았다.

![GET Admin 2](/data/Penetration%20Testing%20%7C%20Week%2012/15.webp)
...
![GET Admin 2](/data/Penetration%20Testing%20%7C%20Week%2012/16.webp)
...
![GET Admin 2](/data/Penetration%20Testing%20%7C%20Week%2012/17.webp)
...
![GET Admin 2](/data/Penetration%20Testing%20%7C%20Week%2012/18.webp)
![GET Admin 2](/data/Penetration%20Testing%20%7C%20Week%2012/19.webp)
...
![GET Admin 2](/data/Penetration%20Testing%20%7C%20Week%2012/20.webp)

별도의 오류 없이 관리자가 접속했다는 메시지가 출력되었다. 이에 따라 관리자 계정의 비밀번호가 `any`로 변경되었을 것이므로, 로그인 페이지로 이동하여 `any_admin`/`any`를 입력하여 로그인을 시도하였다.

![GET Admin 2](/data/Penetration%20Testing%20%7C%20Week%2012/21.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%2012/22.webp" alt="GET Admin 3" style="padding: 0 25%; background-color: white;">

링크의 주소로 접속하여 `any`/`any` 계정을 생성해 로그인한 후, 마이페이지에서 비밀번호를 변경해 보았다.

![GET Admin 3](/data/Penetration%20Testing%20%7C%20Week%2012/23.webp)
![GET Admin 3](/data/Penetration%20Testing%20%7C%20Week%2012/24.webp)
![GET Admin 3](/data/Penetration%20Testing%20%7C%20Week%2012/25.webp)
...
![GET Admin 3](/data/Penetration%20Testing%20%7C%20Week%2012/26.webp)
...
![GET Admin 3](/data/Penetration%20Testing%20%7C%20Week%2012/27.webp)
![GET Admin 3](/data/Penetration%20Testing%20%7C%20Week%2012/28.webp)
...
![GET Admin 3](/data/Penetration%20Testing%20%7C%20Week%2012/29.webp)

별도의 오류 없이 관리자가 접속했다는 메시지가 출력되었다. 이에 따라 관리자 계정의 비밀번호가 `any`로 변경되었을 것이므로, 로그인 페이지로 이동하여 `any_admin`/`any`를 입력하여 로그인을 시도하였다.

![GET Admin 2](/data/Penetration%20Testing%20%7C%20Week%2012/30.webp)
