...
![DOM-Based XSS](/data/Penetration%20Testing%20%7C%20Week%2010/1.webp)

XSS CTF에서와 같은 회원제 게시판 애플리케이션이다. 이때 검색 창에 `test`를 입력하고 검색을 수행하면 다음과 같이 화면에 `test` 텍스트가 출력된다.

![DOM-Based XSS](/data/Penetration%20Testing%20%7C%20Week%2010/2.webp)

따라서 서버 응답에 입력한 검색어가 포함되어 전송되며, XSS 취약점이 존재할 가능성이 있다는 것을 유추할 수 있다.

![DOM-Based XSS](/data/Penetration%20Testing%20%7C%20Week%2010/3.webp)

하지만 Burp Suite를 사용하여 패킷을 확인해 보면 서버 응답에 `test`라는 텍스트는 존재하지 않는다는 것을 알 수 있다. 그렇다면 어떻게 해당 메시지가 화면에 출력된 것일까?

![DOM-Based XSS](/data/Penetration%20Testing%20%7C%20Week%2010/4.webp)
...
![XSS CTF](/data/Penetration%20Testing%20%7C%20Week%2010/5.webp)
...
<img src="/data/Penetration%20Testing%20%7C%20Week%2010/6.webp" alt="XSS 1" style="padding: 0 25%; background-color: white">

취약점 탐색 과정에서 게시판의 글쓰기 페이지(글 제목)에 XSS 취약점이 존재한다는 것을 확인하였다. 따라서 글쓰기 페이지로 이동 후 다음과 같이 악성 스크립트가 포함된 페이로드를 작성하였다.

![XSS 1](/data/Penetration%20Testing%20%7C%20Week%2010/7.webp)
...
![XSS 1](/data/Penetration%20Testing%20%7C%20Week%2010/8.webp)

관리자를 해당 게시물에 접근시키면 관리자의 쿠키 정보를 RequestBin에서 조회할 수 있다. 따라서 우선 해당 게시물을 클릭하여 URL을 확인하였다.

![XSS 1](/data/Penetration%20Testing%20%7C%20Week%2010/9.webp)

게시물 열람 페이지의 URL을 확인한 후, 관리자 방문 Bot 페이지에서 관리자가 해당 URL로 접근하도록 하였다.

![XSS 1](/data/Penetration%20Testing%20%7C%20Week%2010/10.webp)

이후 RequestBin에서 요청이 정상적으로 수신되었고, 쿠키 정보에 플래그가 포함되어 출력되었다.

<img src="/data/Penetration%20Testing%20%7C%20Week%2010/11.webp" alt="XSS 1" style="padding: 0 20%; background-color: white">
...
<img src="/data/Penetration%20Testing%20%7C%20Week%2010/12.webp" alt="XSS 2" style="padding: 0 25%; background-color: white">

취약점 탐색 과정에서 게시판의 검색 창에 XSS 취약점이 존재한다는 것을 확인하였다. 따라서 검색 창에서 다음과 같이 악성 스크립트가 포함된 페이로드를 검색하였다.

![XSS 2](/data/Penetration%20Testing%20%7C%20Week%2010/13.webp)
...
![XSS 2](/data/Penetration%20Testing%20%7C%20Week%2010/14.webp)

검색 요청 URL을 관리자 Bot에게 접속시키면 관리자의 쿠키 정보를 획득할 수 있다. 검색 데이터가 POST 방식으로 전송되고 있었으므로, 요청 방식을 변경한 뒤 URL을 복사하였다.

![XSS 2](/data/Penetration%20Testing%20%7C%20Week%2010/15.webp)

복사한 URL을 관리자 방문 Bot 페이지에 붙여 넣은 후 관리자가 해당 URL로 접근하도록 하였다.

![XSS 2](/data/Penetration%20Testing%20%7C%20Week%2010/16.webp)

이후 RequestBin에서 요청이 정상적으로 수신되었고, 쿠키 정보에 플래그가 포함되어 출력되었다.

<img src="/data/Penetration%20Testing%20%7C%20Week%2010/17.webp" alt="XSS 2" style="padding: 0 20%; background-color: white">
...
<img src="/data/Penetration%20Testing%20%7C%20Week%2010/18.webp" alt="XSS 3" style="padding: 0 25%; background-color: white">

취약점 탐색 과정에서 마이페이지에 XSS 취약점이 존재한다는 것을 확인하였다. 따라서 마이페이지로 이동하여 `user` 파라미터를 다음과 같이 악성 스크립트가 포함된 값으로 변경하였다.

![XSS 3](/data/Penetration%20Testing%20%7C%20Week%2010/19.webp)
...
![XSS 3](/data/Penetration%20Testing%20%7C%20Week%2010/20.webp)
...
![XSS 3](/data/Penetration%20Testing%20%7C%20Week%2010/21.webp)

수정 이후 `<script>` 태그의 내용에 정상적으로 악성 스크립트가 포함된 것을 확인하였다. 이어서 관리자 방문 Bot 페이지에 해당 URL을 입력한 후 관리자가 접근하도록 하였다.

![XSS 3](/data/Penetration%20Testing%20%7C%20Week%2010/22.webp)

이후 RequestBin에서 요청이 정상적으로 수신되었고, 쿠키 정보에 플래그가 포함되어 출력되었다.

<img src="/data/Penetration%20Testing%20%7C%20Week%2010/23.webp" alt="XSS 3" style="padding: 0 20%; background-color: white">
...
<img src="/data/Penetration%20Testing%20%7C%20Week%2010/24.webp" alt="XSS 4" style="padding: 0 25%; background-color: white">

취약점 탐색 과정에서 게시판의 글쓰기 페이지에 XSS 취약점이 존재한다는 것을 확인하였다. 따라서 글쓰기 페이지로 이동 후 다음과 같이 악성 스크립트가 포함된 페이로드를 작성하였다.

![XSS 4](/data/Penetration%20Testing%20%7C%20Week%2010/25.webp)
...
![XSS 4](/data/Penetration%20Testing%20%7C%20Week%2010/26.webp)

게시물 열람 페이지의 URL을 확인한 후, 관리자 방문 Bot 페이지에서 관리자가 해당 URL로 접근하도록 하였다.

![XSS 4](/data/Penetration%20Testing%20%7C%20Week%2010/27.webp)

이후 RequestBin에서 요청이 정상적으로 수신되었고, 쿠키 정보에 플래그가 포함되어 출력되었다.

<img src="/data/Penetration%20Testing%20%7C%20Week%2010/28.webp" alt="XSS 4" style="padding: 0 20%; background-color: white">
...
<img src="/data/Penetration%20Testing%20%7C%20Week%2010/29.webp" alt="XSS 5" style="padding: 0 25%; background-color: white">
...
![XSS 5](/data/Penetration%20Testing%20%7C%20Week%2010/30.webp)

글을 생성한 후, 게시물을 열람하면 서버 응답에 정상적으로 악성 스크립트가 포함됨을 확인하였다.

![XSS 5](/data/Penetration%20Testing%20%7C%20Week%2010/31.webp)

게시물 열람 페이지의 URL을 확인한 후, 관리자 방문 Bot 페이지에서 관리자가 해당 URL로 접근하도록 하였다.

![XSS 5](/data/Penetration%20Testing%20%7C%20Week%2010/32.webp)

이후 RequestBin에서 요청이 정상적으로 수신되었고, 쿠키 정보에 플래그가 포함되어 출력되었다.

<img src="/data/Penetration%20Testing%20%7C%20Week%2010/33.webp" alt="XSS 5" style="padding: 0 20%; background-color: white">
...
<img src="/data/Penetration%20Testing%20%7C%20Week%2010/34.webp" alt="XSS 6" style="padding: 0 25%; background-color: white">

취약점 탐색 과정에서 로그인 페이지에 XSS 취약점이 존재한다는 것을 확인하였다. 따라서 로그인 페이지로 이동 후 다음과 같이 악성 스크립트가 포함된 페이로드를 입력하였다.

![XSS 6](/data/Penetration%20Testing%20%7C%20Week%2010/35.webp)
...
![XSS 6](/data/Penetration%20Testing%20%7C%20Week%2010/36.webp)
...
![XSS 6](/data/Penetration%20Testing%20%7C%20Week%2010/37.webp)

페이로드 수정 후 악성 스크립트가 정상적으로 삽입되는 것을 확인하였다. 로그인 데이터가 POST 방식으로 전송되고 있었으므로, 요청 방식을 변경한 뒤 URL을 복사하였다.

![XSS 6](/data/Penetration%20Testing%20%7C%20Week%2010/38.webp)

복사한 URL을 관리자 방문 Bot 페이지에 붙여 넣은 후 관리자가 해당 URL로 접근하도록 하였다.

![XSS 6](/data/Penetration%20Testing%20%7C%20Week%2010/39.webp)

이후 RequestBin에서 요청이 정상적으로 수신되었고, 쿠키 정보에 플래그가 포함되어 출력되었다.

<img src="/data/Penetration%20Testing%20%7C%20Week%2010/40.webp" alt="XSS 6" style="padding: 0 20%; background-color: white">
