...
URL의 파일 경로는 **Web Root**라 불리는 웹 서버의 루트 디렉토리를 기준으로 하며, 일반적으로는 `/var/www/html`를 사용한다. 즉, 위 예시 경로에서 `index.html`의 실제 위치는 `/var/www/html/index.html`이다.

Web Root보다 상위 디렉토리에 위치한 파일을 요청하면 어떻게 될까?  
예를 들어 브라우저에 `http://192.168.50.177:80/../index.html`과 같은 URL을 입력하면 웹 서버는 해당 파일을 찾을 수 없다는 오류를 반환한다. 이는 클라이언트가 Web Root 하위에 있는 파일에만 접근할 수 있도록 설계되어 있기 때문이다. 만약 Web Root보다 상위 디렉토리에 접근이 가능하다면, 웹 서버가 설치된 시스템의 모든 파일이 외부에 노출될 수 있어 보안상 매우 위험해진다. Web Root로 `/`가 아닌 `/var/www/html`과 같은 제한된 경로를 사용하는 이유도 이 때문이다.
...
이 파일에 접근하려면 `http://[example-ip]/score.php?name=john`과 같은 URL을 이용한다.
...
> 쿼리 스트링의 파라미터가 여러 개인 경우, `name=john&score=100`과 같이 `&`로 구분한다.
...
<span class="hljs-tag">&lt;<span class="hljs-name">html</span>&gt;</span>
  <span class="hljs-tag">&lt;<span class="hljs-name">h1</span>&gt;</span>Score<span class="hljs-tag">&lt;/<span class="hljs-name">h1</span>&gt;</span>
  <span class="hljs-tag">&lt;<span class="hljs-name">h2</span>&gt;</span>Name: john<span class="hljs-tag">&lt;/<span class="hljs-name">h2</span>&gt;</span>
<span class="hljs-tag">&lt;/<span class="hljs-name">html</span>&gt;</span>
</code></pre>
...
`http://[example-ip]/name.php`과 같은 URL로 접속하면 입력 란이 표시된다.

![GET 방식](/data/Penetration%20Testing%20%7C%20Week%201/2.png)

입력 란에 `john`을 입력하고 전송하면, 브라우저는 `http://[example-ip]/name.php?id=john`으로 이동하며 전달된 값이 페이지에 출력된다.
...
`http://[example-ip]/name.php`에 접속하여 입력 란에 `john`을 입력하고 전송하면 다음과 같은 결과를 확인할 수 있다.

![POST 방식](/data/Penetration%20Testing%20%7C%20Week%201/4.png)

`john`이 정상적으로 출력되었으나, **URL에는 변화가 없음**을 알 수 있다.  
...
<span class="hljs-meta">&lt;?php</span>
  ...
  <span class="hljs-keyword">if</span> (<span class="hljs-variable">$username</span> == <span class="hljs-string">"admin"</span> &amp;&amp; <span class="hljs-variable">$password</span> == <span class="hljs-string">"admin1234"</span>) {
...
브라우저에서 `http://[example-ip]/login.php`로 접속하면 다음과 같은 로그인 페이지가 나타난다.
