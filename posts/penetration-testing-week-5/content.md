...
이러한 공격은 주로 로그인, 검색, 필터링 등의 기능에서 발생하며, 성공할 경우 인증 우회, 민감 정보 조회 및 변경, 데이터베이스 구조 노출 등의 피해로 이어질 수 있다. 따라서 SQL Injection은 애플리케이션 보안에서 매우 치명적인 위협 요소 중 하나로 간주된다.

SQL Injection의 동작 원리는 다음과 같다. 로그인 기능을 구현할 때 자주 사용되는 PHP 코드를 생각해 보자.

<style id="code-1">
  #code-1 + pre data[value="1"] span:nth-of-type(15) {color: #032F62 !important;}
</style>
```php
...
<style id="code-2">
  #code-2 + pre data[value="1"] span:nth-of-type(10) {color: #24292E !important;}
</style>
```sql
...
![SQL Injection Practice](/posts/penetration-testing-week-5/assets/2.webp)
...
<style id="code-3">
  #code-3 + pre data[value="1"] span:nth-last-of-type(-n+4) {color: #6A737D !important;}
</style>
```sql
SELECT * FROM user WHERE username = 'user'#' AND password = '1234'
```
...
![SQL Injection Practice](/posts/penetration-testing-week-5/assets/3.webp)
...
![Authentication Bypass](/posts/penetration-testing-week-5/assets/4.webp)
...
![Get Admin](/posts/penetration-testing-week-5/assets/7.webp)

로그인 절차를 분석하기 위해 Burp Suite에서 패킷을 확인하였다.

![Get Admin](/posts/penetration-testing-week-5/assets/8.webp)
![Get Admin](/posts/penetration-testing-week-5/assets/9.webp)
...
![Get Admin](/posts/penetration-testing-week-5/assets/10.webp)
![Get Admin](/posts/penetration-testing-week-5/assets/11.webp)
...
![PIN CODE Bypass](/posts/penetration-testing-week-5/assets/14.webp)

`step1.php`로 이동하며 관리자 확인 화면이 출력되었다. 이후 확인 버튼을 클릭하였다.

![PIN CODE Bypass](/posts/penetration-testing-week-5/assets/15.webp)
...
![PIN CODE Bypass](/posts/penetration-testing-week-5/assets/16.webp)

예상대로 발사 화면이 출력되었다. 정상적으로 인증을 진행하면 `step3.php`로의 리다이렉션이 이루어질 것으로 추측된다.

![PIN CODE Bypass](/posts/penetration-testing-week-5/assets/17.webp)
...
![Admin is Mine](/posts/penetration-testing-week-5/assets/20.webp)
![Admin is Mine](/posts/penetration-testing-week-5/assets/21.webp)
...
![Admin is Mine](/posts/penetration-testing-week-5/assets/22.webp)

예상대로 `result`에 `ok` 값이 저장된 채로 서버에서 응답이 도착하였다.  
다음으로 `admin` 계정으로 무작위 비밀번호와 함께 로그인을 시도하였다. 

![Admin is Mine](/posts/penetration-testing-week-5/assets/23.webp)
...
![Admin is Mine](/posts/penetration-testing-week-5/assets/24.webp)
...
![PIN CODE Crack](/posts/penetration-testing-week-5/assets/31.webp)
...
![PIN CODE Crack](/posts/penetration-testing-week-5/assets/34.webp)
...
![PIN CODE Crack](/posts/penetration-testing-week-5/assets/35.webp)

이에 따라 응답 내용이 달라졌음을 알 수 있었고, `1021`을 입력하면 로그인에 실패하지 않고 `index.php`로 리다이렉트됨을 확인하였다.

![PIN CODE Crack](/posts/penetration-testing-week-5/assets/36.webp)
...
![Login Bypass 1](/posts/penetration-testing-week-5/assets/38.webp)

POST 방식으로 입력한 데이터가 전달되며 `index.php`로 리다이렉트된다. 로그인 실패는 어떤 방식으로 이루어지는지 확인하기 위해 임의의 비밀번호로 로그인을 시도하였다.

![Login Bypass 1](/posts/penetration-testing-week-5/assets/39.webp)
...
<style id="code-4">
  #code-4 + pre data[value="1"] span:nth-of-type(15) {color: #032F62 !important;}
</style>
```php
$sql = "SELECT * FROM user WHERE username = '$username' AND password = '$password'";
```
...
<style id="code-5">
  #code-5 + pre data:is([value="2"], [value="5"]) span:nth-last-of-type(3) {color: #24292E !important;}
</style>
```sql
/* username: user */
SELECT * FROM user WHERE username = 'user' AND password = '1234'
...
![Login Bypass 1](/posts/penetration-testing-week-5/assets/40.webp)
...
![Login Bypass 1](/posts/penetration-testing-week-5/assets/41.webp)

`AND` 연산을 이용한 SQL Injection이 통한다면 `OR` 연산 역시 동일하게 활용할 수 있다.  
`OR` 연산을 사용한 공격은 특히 유용한데, 그 이유는 다음과 같다.

<style id="code-6">
  #code-6 + pre data[value="2"] span:nth-of-type(14) {color: #24292E !important;}
</style>
```sql
...
![Login Bypass 1](/posts/penetration-testing-week-5/assets/44.webp)
...
![Login Bypass 2](/posts/penetration-testing-week-5/assets/48.webp)
...
![Login Bypass 2](/posts/penetration-testing-week-5/assets/49.webp)
...
<style id="code-7">
  #code-7 + pre data[value="9"] span:nth-of-type(15) {color: #032F62 !important;}
</style>
```php
$username = $_POST['UserId'];
$password = $_POST['Password'];
...
![Login Bypass 2](/posts/penetration-testing-week-5/assets/53.webp)

리다이렉션을 통해 `index.php`로 이동하면 플래그가 출력된다.

![Login Bypass 2](/posts/penetration-testing-week-5/assets/54.webp)
