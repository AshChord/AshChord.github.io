...
**SQL Injection**은 웹 애플리케이션에서 사용자의 입력값이 적절한 검증 없이 SQL 쿼리에 직접 포함될 때 발생하는 보안 취약점을 말한다. 공격자는 이 취약점을 이용해 악의적으로 조작된 SQL 구문을 삽입함으로써, 애플리케이션이 원래 의도한 동작과는 다른 방식으로 데이터베이스 명령을 수행하도록 유도할 수 있다.
...
![SQL Injection Practice](/posts/Penetration%20Testing%20%7C%20Week%205/1.webp)

위의 웹 사이트는 앞서 살펴보았던 PHP 코드를 사용하여 구현된 간이 로그인 사이트이다.  
`user'`와 `1234`를 입력하여 로그인을 시도해 보자.

![SQL Injection Practice](/posts/Penetration%20Testing%20%7C%20Week%205/2.webp)

단순히 로그인에 실패하는 것이 아니라, SQL 문법 오류(SQL syntax error)가 발생한다. 다시 말해 서버가 생성한 쿼리가 SQL 문법에 맞지 않는다는 의미이다.  
그 이유는 간단한데, 사용자의 입력값이 쿼리에 그대로 삽입되면서 최종적으로 다음과 같은 형태가 되기 때문이다.

<pre><button class="copy-button"></button><code class="language-sql" highlighted><data class="code-line" value="1"><span class="hljs-keyword">SELECT</span> <span class="hljs-operator">*</span> <span class="hljs-keyword">FROM</span> user <span class="hljs-keyword">WHERE</span> username <span class="hljs-operator">=</span> <span class="hljs-string">'user'' AND password = '</span>1234'
</data></code></pre>
...
![SQL Injection Practice](/posts/Penetration%20Testing%20%7C%20Week%205/3.webp)
...
![Authentication Bypass](/posts/Penetration%20Testing%20%7C%20Week%205/4.webp)
...
<img src="/posts/Penetration%20Testing%20%7C%20Week%205/5.webp" alt="Get Admin" style="padding: 0 25%; background-color: white">

링크의 주소로 접속하면 다음과 같은 웹 페이지로 이동한다.

![Get Admin](/posts/Penetration%20Testing%20%7C%20Week%205/6.webp)

우선 주어진 계정으로 로그인을 시도하였다.

![Get Admin](/posts/Penetration%20Testing%20%7C%20Week%205/7.webp)

로그인 절차를 분석하기 위해 Burp Suite에서 패킷을 확인하였다.

![Get Admin](/posts/Penetration%20Testing%20%7C%20Week%205/8.webp)
![Get Admin](/posts/Penetration%20Testing%20%7C%20Week%205/9.webp)
...
![Get Admin](/posts/Penetration%20Testing%20%7C%20Week%205/10.webp)
![Get Admin](/posts/Penetration%20Testing%20%7C%20Week%205/11.webp)
...
<img src="/posts/Penetration%20Testing%20%7C%20Week%205/12.webp" alt="PIN CODE Bypass" style="padding: 0 25%; background-color: white">

링크의 주소로 접속하면 다음과 같은 웹 페이지로 이동한다.

![PIN CODE Bypass](/posts/Penetration%20Testing%20%7C%20Week%205/13.webp)

우선 Fire 버튼을 클릭해 보았다.

![PIN CODE Bypass](/posts/Penetration%20Testing%20%7C%20Week%205/14.webp)

`step1.php`로 이동하며 관리자 확인 화면이 출력되었다. 이후 확인 버튼을 클릭하였다.

![PIN CODE Bypass](/posts/Penetration%20Testing%20%7C%20Week%205/15.webp)
...
![PIN CODE Bypass](/posts/Penetration%20Testing%20%7C%20Week%205/16.webp)

예상대로 발사 화면이 출력되었다. 정상적으로 인증을 진행하면 `step3.php`로의 리다이렉션이 이루어질 것으로 추측된다.

![PIN CODE Bypass](/posts/Penetration%20Testing%20%7C%20Week%205/17.webp)
...
<img src="/posts/Penetration%20Testing%20%7C%20Week%205/18.webp" alt="Admin is Mine" style="padding: 0 25%; background-color: white">
...
<img src="/posts/Penetration%20Testing%20%7C%20Week%205/19.webp" alt="Admin is Mine" style="padding: 0 25%; background-color: #2b2b2b">

`bootstrap.min.js`는 웹 디자인 관련 코드로 중요하지 않은 내용이었고, `login.js`는 로그인 절차와의 연관성이 분명해 보였으므로 패킷을 자세히 확인해 보았다.

![Admin is Mine](/posts/Penetration%20Testing%20%7C%20Week%205/20.webp)
![Admin is Mine](/posts/Penetration%20Testing%20%7C%20Week%205/21.webp)
...
![Admin is Mine](/posts/Penetration%20Testing%20%7C%20Week%205/22.webp)

예상대로 `result`에 `ok` 값이 저장된 채로 서버에서 응답이 도착하였다.  
다음으로 `admin` 계정으로 무작위 비밀번호와 함께 로그인을 시도하였다. 

![Admin is Mine](/posts/Penetration%20Testing%20%7C%20Week%205/23.webp)
...
![Admin is Mine](/posts/Penetration%20Testing%20%7C%20Week%205/24.webp)

가로챈 요청을 우클릭하여 컨텍스트 메뉴를 열면 해당 요청에 대한 응답을 가로챌 수 있다.  
바로 해당 방법을 적용해 실행해 보았다.

![Admin is Mine](/posts/Penetration%20Testing%20%7C%20Week%205/25.webp)
![Admin is Mine](/posts/Penetration%20Testing%20%7C%20Week%205/26.webp)

정상적으로 응답을 가로챌 수 있었고, 이를 변조하여 `fail`을 `ok`로 수정하면 로그인에 성공할 것으로 추측하였다.

![Admin is Mine](/posts/Penetration%20Testing%20%7C%20Week%205/27.webp)
![Admin is Mine](/posts/Penetration%20Testing%20%7C%20Week%205/28.webp)
...
<img src="/posts/Penetration%20Testing%20%7C%20Week%205/29.webp" alt="PIN CODE Crack" style="padding: 0 25%; background-color: white">

링크의 주소로 접속하면 다음과 같은 웹 페이지로 이동한다.

![PIN CODE Crack](/posts/Penetration%20Testing%20%7C%20Week%205/30.webp)

LOGIN 버튼을 누르면 다음과 같이 4자리 PIN Code를 입력하라는 화면이 출력된다.

![PIN CODE Crack](/posts/Penetration%20Testing%20%7C%20Week%205/31.webp)

임의로 `0000`을 입력한 후 Burp Suite에서 패킷을 확인해 보았다.

![PIN CODE Crack](/posts/Penetration%20Testing%20%7C%20Week%205/32.webp)
...
![PIN CODE Crack](/posts/Penetration%20Testing%20%7C%20Week%205/33.webp)
...
![PIN CODE Crack](/posts/Penetration%20Testing%20%7C%20Week%205/34.webp)
...
![PIN CODE Crack](/posts/Penetration%20Testing%20%7C%20Week%205/35.webp)

이에 따라 응답 내용이 달라졌음을 알 수 있었고, `1021`을 입력하면 로그인에 실패하지 않고 `index.php`로 리다이렉트됨을 확인하였다.

![PIN CODE Crack](/posts/Penetration%20Testing%20%7C%20Week%205/36.webp)
...
for i in range(10000):
  code = f"{i:04}"
  response = requests.get(url, params={"otpNUM": code})
...
<img src="/posts/Penetration%20Testing%20%7C%20Week%205/37.webp" alt="Login Bypass 1" style="padding: 0 25%; background-color: white">

링크의 주소로 접속하면 예상대로 로그인 페이지로 이동한다.  

우선 주어진 계정으로 로그인을 시도해 보았다.

![Login Bypass 1](/posts/Penetration%20Testing%20%7C%20Week%205/38.webp)

POST 방식으로 입력한 데이터가 전달되며 `index.php`로 리다이렉트된다. 로그인 실패는 어떤 방식으로 이루어지는지 확인하기 위해 임의의 비밀번호로 로그인을 시도하였다.

![Login Bypass 1](/posts/Penetration%20Testing%20%7C%20Week%205/39.webp)

로그인에 실패하는 경우는 상태 코드 `200 OK`가 도착하며 `index.php`로 이동하지 않는다. 이외에는 특이 사항이 보이지 않아 이번 주 학습한 SQL Injection을 시도해 보기로 결정하였다.
...
<pre><button class="copy-button"></button><code class="language-sql" highlighted><data class="code-line" value="1"><span class="hljs-comment">/* username: user */</span>
</data><data class="code-line" value="2"><span class="hljs-keyword">SELECT</span> <span class="hljs-operator">*</span> <span class="hljs-keyword">FROM</span> user <span class="hljs-keyword">WHERE</span> username <span class="hljs-operator">=</span> <span class="hljs-string">'user'</span> <span class="hljs-keyword">AND</span> password <span class="hljs-operator">=</span> <span class="hljs-string">'1234'</span>
</data><data class="code-line" value="3">
</data><data class="code-line" value="4"><span class="hljs-comment">/* username: user' and '1' = '1 */</span>
</data><data class="code-line" value="5"><span class="hljs-keyword">SELECT</span> <span class="hljs-operator">*</span> <span class="hljs-keyword">FROM</span> user <span class="hljs-keyword">WHERE</span> username <span class="hljs-operator">=</span> <span class="hljs-string">'user'</span> <span class="hljs-keyword">and</span> <span class="hljs-string">'1'</span> <span class="hljs-operator">=</span> <span class="hljs-string">'1'</span> <span class="hljs-keyword">AND</span> password <span class="hljs-operator">=</span> <span class="hljs-string">'1234'</span>
</data><data class="code-line" value="6">
</data><data class="code-line" value="7"><span class="hljs-comment">/* SQL 키워드는 가독성을 위해 대문자로 작성하는 것이 관례이나, 문법적으로 대소문자를 구분하지 않음 */</span>
</data></code></pre>
...
![Login Bypass 1](/posts/Penetration%20Testing%20%7C%20Week%205/40.webp)
...
![Login Bypass 1](/posts/Penetration%20Testing%20%7C%20Week%205/41.webp)

`AND` 연산을 이용한 SQL Injection이 통한다면 `OR` 연산 역시 동일하게 활용할 수 있다.  
`OR` 연산을 사용한 공격은 특히 유용한데, 그 이유는 다음과 같다.

<pre><button class="copy-button"></button><code class="language-sql" highlighted><data class="code-line" value="1"><span class="hljs-comment">/* username: user' or '1' = '1 */</span>
...
![Login Bypass 1](/posts/Penetration%20Testing%20%7C%20Week%205/42.webp)

정확한 로그인 절차와 사용되는 SQL 쿼리는 확인할 수 없으나, 상기한 방식과 유사한 처리가 이루어지고 있음을 추측할 수 있다.
이제 ID만 `normaltic1`으로 바꾸면 `normaltic1` 계정으로 로그인할 수 있다.

![Login Bypass 1](/posts/Penetration%20Testing%20%7C%20Week%205/43.webp)
...
<img src="/posts/Penetration%20Testing%20%7C%20Week%205/45.webp" alt="Login Bypass 2" style="padding: 0 25%; background-color: white">

링크의 주소로 접속하여 로그인을 시도해 보았다.

![Login Bypass 2](/posts/Penetration%20Testing%20%7C%20Week%205/46.webp)
![Login Bypass 2](/posts/Penetration%20Testing%20%7C%20Week%205/47.webp)

Login Bypass 1과 동일하게, 로그인에 성공하면 `index.php`로 리다이렉트되고 실패하면 `200 OK`가 출력된다.  
마찬가지로 SQL Injection을 시도해 보았다.

![Login Bypass 2](/posts/Penetration%20Testing%20%7C%20Week%205/48.webp)
...
![Login Bypass 2](/posts/Penetration%20Testing%20%7C%20Week%205/49.webp)
...
![Login Bypass 2](/posts/Penetration%20Testing%20%7C%20Week%205/50.webp)
...
![Login Bypass 2](/posts/Penetration%20Testing%20%7C%20Week%205/51.webp)
...
위와 같은 경우 입력값에 `OR` 연산자를 포함시키는 즉시 로그인은 실패로 처리된다. 이러한 전제 하에, `OR` 연산을 활용한 공격이 아닌 주석 처리를 이용한 공격을 시도해 보았다.

![Login Bypass 2](/posts/Penetration%20Testing%20%7C%20Week%205/52.webp)
...
![Login Bypass 2](/posts/Penetration%20Testing%20%7C%20Week%205/53.webp)
