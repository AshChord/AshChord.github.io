...
![Level 0](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/1.webp)

비밀번호 `bandit0`을 입력한다.

![Level 0](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/2.webp)
...
![Level 1](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/3.webp)

`readme` 파일의 내용을 출력하기 위해 `cat readme`를 입력한다.

![Level 1](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/4.webp)

`bandit1` 계정의 비밀번호가 출력되었다.

![Level 1](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/5.webp)
...
![Level 2](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/6.webp)

`-` 파일의 내용을 출력하기 위해 `cat -`를 입력해 보았다.

![Level 2](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/7.webp)
...
![Level 2](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/8.webp)

파일명이 `-`인 경우, 즉 `cat -` 명령을 사용하면 표준 입력을 읽어 출력한다고 한다.

![Level 2](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/9.webp)
...
![Level 2](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/10.webp)
...
![Level 2](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/11.webp)

지금까지 확인한 내용을 바탕으로, 터미널에 `cat ./-`를 입력하였다.

![Level 2](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/12.webp)

`bandit2` 계정의 비밀번호가 출력되었다.

![Level 2](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/13.webp)
...
![Level 3](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/14.webp)

파일명이 `-`로 시작하므로, 경로를 명시하여 `cat ./--spaces in this filename--`의 형태로 파일 내용 출력을 시도해 보았다.

![Level 3](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/15.webp)
...
![Level 3](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/16.webp)

공백 문자를 역슬래시(`\`) 문자를 사용해 이스케이프하거나, 파일명 전체를 따옴표로 감싸는 방식을 사용할 수 있다고 한다. 이에 따라 터미널에 `cat ./--spaces\ in\ this\ filename--`를 입력하였다.

![Level 3](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/17.webp)

`bandit3` 계정의 비밀번호가 출력되었다.

![Level 3](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/18.webp)
...
![Level 4](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/19.webp)

`cd inhere`를 입력하여 `inhere` 디렉터리로 이동한 후 `ls` 명령어를 입력해 보았다.

![Level 4](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/20.webp)

실행 결과, 출력되는 내용이 존재하지 않았다. `inhere` 디렉터리에 숨김 파일이 위치하고 있으므로, `ls -a` 명령어를 사용하여 숨김 파일을 포함한 전체 파일 및 디렉터리 목록을 출력해 보았다.

![Level 4](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/21.webp)

`.`로 시작하는 숨김 파일인 `...Hiding-From-You` 파일을 확인하였다. 이후 해당 파일의 내용을 출력하기 위해 `cat ...Hiding-From-You`를 입력하였다.

![Level 4](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/22.webp)

`bandit4` 계정의 비밀번호가 출력되었다.

![Level 4](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/23.webp)
...
![Level 5](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/24.webp)

`cd inhere`를 입력하여 `inhere` 디렉터리로 이동한 후 `ls` 명령어를 입력해 보았다.

![Level 5](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/25.webp)

총 10개의 파일이 확인되었다. 이 중 사람이 읽을 수 있는 형식의 파일이 유일하게 존재하며, 해당 파일에 비밀번호가 저장되어 있다. 따라서 `cat ./*` 명령어를 사용하여 모든 파일의 내용을 출력해 보았다.

![Level 5](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/26.webp)

알 수 없는 데이터 사이에서 `bandit5` 계정의 비밀번호가 출력되는 것을 확인할 수 있다. 보다 정확한 확인을 위해 `file ./*` 명령어를 사용하여 각 파일이 어떤 유형인지 출력해 보았다.

![Level 5](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/27.webp)

`-file07` 파일만이 유일하게 일반적인 ASCII 텍스트로 작성된 파일임을 알 수 있다.

![Level 5](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/28.webp)

`cat ./-file07`을 입력하면 `bandit5` 계정의 비밀번호를 재차 확인할 수 있다.

![Level 5](/posts/OverTheWire%20Bandit%20%7C%20Level%200%20~%205/29.webp)
