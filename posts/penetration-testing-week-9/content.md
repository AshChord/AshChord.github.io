...
특수 문자가 정상적으로 처리되는 경우, 공격자는 서버에 악성 스크립트를 삽입할 수 있다. 일반적으로 사용되는 방법은 HTML의 `<script>` 태그를 활용한 방식으로, `<script>` 태그는 HTML 문서에 JavaScript 코드를 포함하기 위해 사용된다. 공격자는 이 태그를 활용하여 악성 자바스크립트 코드를 삽입하며, 예를 들어 `<script>alert(1);</script>`와 같은 형태로 페이로드를 삽입할 수 있다. 해당 페이로드가 포함된 페이지를 로드한 사용자의 브라우저에서는 JavaScript 코드가 실행되며, 공격자는 경고 창을 띄우거나, 더 심각한 경우 쿠키 탈취, 악성 링크 실행 등의 결과를 유도할 수 있다.
...
![XSS CTF](/posts/penetration-testing-week-9/assets/1.webp)
...
![XSS 1](/posts/penetration-testing-week-9/assets/2.webp){:style="padding: 0 25%; background-color: white"}

링크를 통해 접속하면 SQL Injection Advanced CTF에서와 같은 회원제 게시판 애플리케이션으로 이동한다.

![XSS 1](/posts/penetration-testing-week-9/assets/3.webp)

XSS 공격의 특성을 고려하면, 작성한 글이 화면에 출력되는 게시판에 취약점이 존재할 확률이 높다. 따라서 우선 회원가입을 진행해 `any`/`any` 계정을 생성한 후, 로그인하여 게시판에 글을 작성해 보았다.

![XSS 1](/posts/penetration-testing-week-9/assets/4.webp)

게시판의 동작 방식에 따라 입력한 데이터가 자연스럽게 게시물 목록에 출력되는 것을 알 수 있다.

![XSS 1](/posts/penetration-testing-week-9/assets/5.webp)
![XSS 1](/posts/penetration-testing-week-9/assets/6.webp)
...
![XSS 1](/posts/penetration-testing-week-9/assets/7.webp)
...
![XSS 1](/posts/penetration-testing-week-9/assets/8.webp)
...
![XSS 1](/posts/penetration-testing-week-9/assets/9.webp)

서버 응답에 스크립트가 정상적으로 포함되어 있는 것을 확인하였다.

![XSS 1](/posts/penetration-testing-week-9/assets/10.webp)
...
![XSS 2](/posts/penetration-testing-week-9/assets/11.webp){:style="padding: 0 25%; background-color: white"}

링크를 통해 접속한 후, 우선 제목과 내용이 `<'">`인 게시물을 생성해 보았다.

![XSS 2](/posts/penetration-testing-week-9/assets/12.webp)
![XSS 2](/posts/penetration-testing-week-9/assets/13.webp)
...
![XSS 2](/posts/penetration-testing-week-9/assets/14.webp)

그러나 검색어는 화면에 출력되지 않았다. 따라서 예상했던 동작 방식이 실제 구현과 다르다는 사실을 확인하였으나, 검색 결과가 반환되지 않은 점에 의문이 들어 패킷을 확인해 보았다.

![XSS 2](/posts/penetration-testing-week-9/assets/15.webp)
...
<style id="code-1">
  #code-1 + pre data[value="3"] span:is(:nth-of-type(1), :nth-of-type(3)) {color: #24292E !important;}
  #code-1 + pre data[value="3"] span:nth-of-type(2) {color: #22863A !important;}
</style>
```html
<script>
...
알림 창이 표시되지 않은 것은 검색어에 포함된 작은따옴표(`'`) 및 큰따옴표(`"`) 문자가 별도의 변환 처리 없이 그대로 전달되었기 때문이었다. 위의 스크립트는 `alert()` 함수의 인자로 전달된 문자열을 알림 창에 출력하는데, 문자열 내에 삽입된 작은따옴표로 인해 문자열이 올바르게 종료되지 않았고, 그로 인해 문법 오류가 발생하여 스크립트가 정상적으로 실행되지 않았던 것이다.
    ...
    window.patchCodeLine = (lineNumber, override) => {
      ...
      const patch = () => {
        if (code.classList.contains('language-text')) {
          code.classList.replace('language-text', 'language-js');
        }
...
![XSS 2](/posts/penetration-testing-week-9/assets/16.webp)
...
![XSS 2](/posts/penetration-testing-week-9/assets/17.webp)

요청 전송 시 우클릭을 통해 컨텍스트 메뉴를 열면 요청 방식을 변경할 수 있는 항목이 존재한다.

![XSS 2](/posts/penetration-testing-week-9/assets/18.webp)

요청 방식을 GET 방식으로 변경하여 전송하면 정상적으로 서버 응답이 반환되는 것을 확인할 수 있으며, 해당 요청의 URL 역시 추출할 수 있다.

![XSS 2](/posts/penetration-testing-week-9/assets/19.webp)
![XSS 2](/posts/penetration-testing-week-9/assets/20.webp)
...
![XSS 3](/posts/penetration-testing-week-9/assets/21.webp){:style="padding: 0 25%; background-color: white"}
...
![XSS 3](/posts/penetration-testing-week-9/assets/22.webp)
...
![XSS 3](/posts/penetration-testing-week-9/assets/23.webp)
...
<style id="code-1">
  #code-1 + pre data[value="1"] span:nth-of-type(10) {color: #24292E !important; font-style: normal !important;}
</style>
```html
...
<"/> // 유효한 HTML 태그 형태가 아니므로 HTML 요소로 해석되지 않고 일반 텍스트로 표시됨
```
<script>
  patchCodeLine(5, [
    '<span style="color:#24292E">&lt;"/&gt; </span>',
    '<span style="color:#6A737D">// 유효한 HTML 태그 형태가 아니므로 HTML 요소로 해석되지 않고 일반 텍스트로 표시됨</span>'
  ].join(''));
</script>

![XSS 3](/posts/penetration-testing-week-9/assets/24.webp)

서버 응답에 스크립트가 정상적으로 포함되는 것을 확인하였다. 이 방식 또한 Reflected XSS에 해당하므로, 해당 요청의 URL로 접속해 스크립트가 실행되는지 재확인해 보았다.

![XSS 3](/posts/penetration-testing-week-9/assets/25.webp)
...
![XSS 4](/posts/penetration-testing-week-9/assets/26.webp){:style="padding: 0 25%; background-color: white"}

링크를 통해 접속하여 제목과 내용이 `<'">`인 게시물을 생성해 보았다.

![XSS 4](/posts/penetration-testing-week-9/assets/27.webp)
![XSS 4](/posts/penetration-testing-week-9/assets/28.webp)
...
![XSS 4](/posts/penetration-testing-week-9/assets/29.webp)
...
![XSS 4](/posts/penetration-testing-week-9/assets/30.webp)

예상한 대로 `<script>` 태그가 정상적으로 반환되었다. 다음으로 `<scrscriptipt>alert(1);</scrscriptipt>`을 사용하여 스크립트 실행을 시도하였다. 

![XSS 4](/posts/penetration-testing-week-9/assets/31.webp)

`alert` 문자열 역시 필터링되고 있음을 확인하였다. 마찬가지 방법으로 `<scrscriptipt>alalertert(1);</scrscriptipt>`를 대신 사용하였다.

![XSS 4](/posts/penetration-testing-week-9/assets/32.webp)

서버 응답에 스크립트가 정상적으로 포함되어 있는 것을 확인하였다.

![XSS 4](/posts/penetration-testing-week-9/assets/33.webp)
...
![XSS 5](/posts/penetration-testing-week-9/assets/34.webp){:style="padding: 0 25%; background-color: white"}
...
![XSS 5](/posts/penetration-testing-week-9/assets/35.webp)
...
![XSS 5](/posts/penetration-testing-week-9/assets/36.webp)
![XSS 5](/posts/penetration-testing-week-9/assets/37.webp)
...
![XSS 5](/posts/penetration-testing-week-9/assets/38.webp)

서버 응답에 스크립트가 정상적으로 포함되어 있는 것을 확인하였다.

![XSS 5](/posts/penetration-testing-week-9/assets/39.webp)
...
![XSS 6](/posts/penetration-testing-week-9/assets/40.webp){:style="padding: 0 25%; background-color: white"}

링크를 통해 접속하여 게시판 및 마이페이지를 면밀히 조사해 보았지만 XSS 취약점을 발견할 수 없었다.

마지막으로 XSS 공격의 가능성을 확인한 곳은 다름 아닌 `index.php` 페이지였다.

![XSS 6](/posts/penetration-testing-week-9/assets/41.webp)

로그인을 수행하면 사용자의 아이디가 화면에 출력되고 있었다. 따라서, 회원 가입 페이지에서 아이디가 `<script>alert(1);</script>`인 계정을 만든 후 로그인하면 XSS 공격이 가능할 것으로 판단하였다.
...
![XSS 6](/posts/penetration-testing-week-9/assets/42.webp)

회원 가입에 성공하였다. 이후 로그인 페이지에서 `<script>alert(1);</script>`/`any`를 입력하여 로그인을 시도하였다.

![XSS 6](/posts/penetration-testing-week-9/assets/43.webp)

로그인을 시도하자 서버 응답에 `alert()` 함수가 포함된 스크립트가 반환되었다. 응답 내용의 구조를 고려하면, 입력한 아이디가 `alert('[`와 `] 등록되지 않은 사용자입니다.')` 사이에 삽입된다는 사실을 파악할 수 있었다. 회원 가입에 성공했기 때문에 해당 아이디 `<script>alert(1);</script>`는 데이터베이스에 등록되었겠지만, 일반적이지 않은 값이 입력되었기 때문에 내부적으로 코드가 정상적으로 동작하지 않아 미등록 아이디 안내 창이 표시된 것으로 보인다.
...
![XSS 6](/posts/penetration-testing-week-9/assets/44.webp)

서버 응답에 스크립트가 정상적으로 포함되는 것을 확인하였다. 이 방식 또한 Reflected XSS에 해당하므로, 해당 요청의 URL로 접속해 스크립트가 실행되는지 재확인해 보았다.

![XSS 6](/posts/penetration-testing-week-9/assets/45.webp)
![XSS 6](/posts/penetration-testing-week-9/assets/46.webp)
